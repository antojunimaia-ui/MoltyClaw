...
# Hub compartilhado — mesma instância do agente usada pela WebUI, Discord e outros canais
hub = get_hub()


async def post_init(application) -> None:
    bot_info = await application.bot.get_me()
    console.print(f"[bold blue]🤖 Telegram conectado como @{bot_info.username}![/bold blue]")
    console.print("[bold green]Telegram conectado ao AgentHub compartilhado (aguardando mensagens)...[/bold green]")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if allowed_users.strip():
        ...
        if peer_id not in allowed_list and username.replace("@", "") not in [u.replace("@", "") for u in allowed_list]:
            console.print(f"[bold yellow][Segurança] Ignorando Telegram de não autorizado: {username} ({peer_id})[/bold yellow]")
            return
    ...
    if update.message.text:
        ...
        console.print(f"[info]🎧 Áudio do Telegram recebido, transcrevendo...[/info]")
...
if __name__ == '__main__':
    if not TELEGRAM_TOKEN:
        ...
        try:
            ...
            console.print(f"[bold red]❌ Erro fatal ao iniciar o bot do Telegram: {e}[/bold red]")
            sys.exit(1)
